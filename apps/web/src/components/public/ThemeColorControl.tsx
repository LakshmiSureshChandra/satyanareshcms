...
const THEMES = [
  { key: 'default', label: 'Midnight', paper: '#0b1220', ink: '#f8fafc' },
  { key: 'light', label: 'Daylight', paper: '#f8fafc', ink: '#0f172a' },
            ...
            {THEMES.map((t) => (
              <button
                ...
                className={`flex w-full items-center gap-2.5 rounded-lg px-2 py-2 text-sm transition-colors ${
                  theme === t.key ? 'bg-paper-2 font-semibold text-ink' : 'text-ink-2 hover:bg-paper-2'
                }`}
              >
                <span className="size-5 shrink-0 rounded-full" style={swatch(t.paper, t.ink)} />
                {t.label}
              </button>
            ))}
          </div>
