...
export default function MenusPage() {
  ...
  function addItem(e: React.FormEvent) {
    ...
    const title = form.title || ref?.title || ref?.name || ''
    if (!title) return
    setItems([
      ...items,
      {
  ...
  function onDragEnd(e: DragEndEvent) {
    ...
    setItems((list) => {
      ...
      // clamp depth: an item can be at most one deeper than the one above it
  ...
  async function save() {
    setBusy(true)
    await adminApi('/admin/menus', { method: 'PUT', body: { tree: toTree(items) } })
    setBusy(false)
    setSaved(true)
        ...
        <form onSubmit={addItem} className="h-fit space-y-3 rounded-xl border border-stone-200 bg-white p-5">
          ...
          {form.type === 'custom' ? (
            <>
              ...
              <div>
                <label className="admin-label">URL</label>
                <input value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} required placeholder="/contact or https://…" className="admin-input" />
              </div>
              <label className="flex items-center gap-2 text-sm">
          ...
          <button className="w-full rounded-md border border-stone-300 py-2 text-sm font-semibold hover:bg-stone-100">+ Add to menu</button>
        </form>

        {/* tree */}
        <div className="lg:col-span-2">
          <p className="mb-3 text-xs text-stone-500">Drag ⠿ to reorder · use ← → to nest (up to 3 levels) · remember to Save</p>
          ...
          {!items.length && <p className="rounded-xl border border-dashed border-stone-300 py-10 text-center text-sm text-stone-400">Menu is empty — add items from the left.</p>}
        </div>
      </div>
    </div>
  )
